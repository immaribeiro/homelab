...
- **Cron data helper (2026-08-21):** `~/.hermes/profiles/investor/scripts/fetch_briefing_data.py` — scan-safe prices+news fetcher used by the daily briefing / weekly review / CLARITY check prompts. **Do not embed `curl https://...` in cron prompts** — TIRITH blocks prompts matching `exfil_curl_url` (curl + URL + `$KEY/TOKEN/API` var). All investor cron prompts were rewritten to call the helper (2026-08-21).
