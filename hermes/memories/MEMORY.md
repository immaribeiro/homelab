...
§
Homelab: Mac Mini (imma-mini, 100.101.63.91) M4 24GB. Hermes hub, gateway multiplex_profiles enabled. 3 agent profiles: architect (DeepSeek R1 via Nous), backend (Qwen3 Coder 30B via Nous), frontend (Qwen 3.5 9B via LMStudio local). LMStudio models: qwen3.5-9b-mlx, google/gemma-4-e4b. Telegram bot @ImmaHermesBot, forum group 'Hermes' (chat_id -1004449482428) routes by thread_id: General(1)→main, Architecture(3)→architect, Backend(4)→backend, Frontend(5)→frontend. All Hermes configs symlinked from ~/.hermes/ to ~/GitHub/homelab/hermes/ for git sync. Obsidian vault at ~/GitHub/homelab/hermes/vault/, OBSIDIAN_VAULT_PATH set in .env. Vault shared via SMB ('Hermes Vault' share) over Tailscale for remote access from MacBook Air. Single git folder at ~/GitHub/ (consolidated from 3 locations).
§
